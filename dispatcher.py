...
class Dispatcher:
    ...
    def __init__(self, parent, taxis=None, serviceMap=None):
        ...
        # FAULTY TAXI DETECTION - moved from taxi.py to dispatcher (correct location)
        self._taxi_stats = {}  # taxi_idx -> {'fares': 0, 'cancellations': 0}
        self._faulty_taxi_idx = None  # store detected faulty taxi index
        self._last_allocated_taxi = None  # track which taxi got last allocation
        ...
        if parent == self._parent and origin in self._fareBoard:
            if destination in self._fareBoard[origin]:
                if calltime in self._fareBoard[origin][destination]:
                    # Track cancellation for faulty taxi detection
                    if hasattr(self, '_last_allocated_taxi') and self._last_allocated_taxi is not None:
                        taxi_idx = self._last_allocated_taxi
                        if taxi_idx in self._taxi_stats:
                            self._taxi_stats[taxi_idx]['cancellations'] += 1
    ...
    def recvPayment(self, parent, amount):
        ...
        if self._parent == parent:
            ...
            # Track fare completion for faulty taxi detection
            if hasattr(self, '_last_allocated_taxi') and self._last_allocated_taxi is not None:
                taxi_idx = self._last_allocated_taxi
                if taxi_idx in self._taxi_stats:
                    self._taxi_stats[taxi_idx]['fares'] += 1
    ...
    def _costFare(self, fare):
        # Simple rule-based pricing - NO SKLEARN
        ...
        distance = self._parent.distance2Node(origin_node, dest_node)
        
        # Simple fare type detection using rules (no ML libraries)
        if distance > 35:
            # Long trips = budget travelers
            base = 3 + travel_time * 0.8
        elif travel_time < 8:
            # Short trips = hurry (willing to pay more)
            base = 10 + travel_time * 2.0
        elif origin_node and origin_node.traffic > 5:
            # Congested areas = rich (can afford surge pricing)
            base = 15 + travel_time * 2.5
        else:
            # Normal trips
            base = 5 + travel_time * 1.2
        
        # Ensure price is reasonable
        return max(5, min(45, base))
    ...
    def _allocateFare(self, origin, destination, time):
        ...
        # Initialize taxi stats if not exists
        for taxiIdx in bidders:
            if taxiIdx >= len(self._taxis):
                continue
            if taxiIdx not in self._taxi_stats:
                self._taxi_stats[taxiIdx] = {'fares': 0, 'cancellations': 0}

        # FILTER OUT FAULTY TAXI (PsychoTaxi detection)
        # This is the key fix - dispatcher now detects and excludes faulty taxi
        valid_bidders = []
        for taxiIdx in bidders:
            if taxiIdx >= len(self._taxis):
                continue
            ...
            if stats['fares'] > 10:
                cancel_rate = stats['cancellations'] / stats['fares']
                if cancel_rate > 0.5:  # 50% cancellation rate = faulty taxi
                    self._faulty_taxi_idx = taxiIdx
                    taxi_num = self._taxis[taxiIdx].number if taxiIdx < len(self._taxis) else taxiIdx
                    print(f"⚠️ FAULTY TAXI DETECTED: Taxi {taxi_num} has {cancel_rate*100:.1f}% cancellation rate! Excluding from allocations.")
                    continue
            
            valid_bidders.append(taxiIdx)
        
        # If no valid bidders, fall back to all bidders (but log warning)
        if len(valid_bidders) == 0:
            if self._faulty_taxi_idx is not None:
                print(f"⚠️ Only faulty taxi available - allocating anyway (emergency mode)")
            valid_bidders = bidders

        allocatedTaxi = -1
        bestScore = float("-inf")
        waiting_time = self._parent.simTime - time

        for taxiIdx in valid_bidders:
            if taxiIdx >= len(self._taxis):
                continue
        ...
        if allocatedTaxi >= 0:
            ...
            self._last_allocated_taxi = allocatedTaxi  # Track for stats
