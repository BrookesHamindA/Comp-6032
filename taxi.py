...
# -------------------------------------------------------------------------------------
class FaultyTaxiTracker:
    """Tracks taxi performance to identify faulty taxi"""
    
    def __init__(self):
        self.stats = {}  # taxi_num -> {fares, revenue, cancellations}
        
    def record_fare_completed(self, taxi_num, revenue):
        if taxi_num not in self.stats:
            self.stats[taxi_num] = {'fares': 0, 'revenue': 0, 'cancellations': 0}
        self.stats[taxi_num]['fares'] += 1
        self.stats[taxi_num]['revenue'] += revenue
        
    def record_cancellation(self, taxi_num):
        if taxi_num not in self.stats:
            self.stats[taxi_num] = {'fares': 0, 'revenue': 0, 'cancellations': 0}
        self.stats[taxi_num]['cancellations'] += 1
        
    def identify_faulty(self):
        """Returns the taxi number that performs worst"""
        if not self.stats:
            return None
            
        # Calculate average fares
        total_fares = sum(s['fares'] for s in self.stats.values())
        avg_fares = total_fares / len(self.stats)
        
        # Find taxi with significantly fewer fares
...
faulty_tracker = FaultyTaxiTracker()
