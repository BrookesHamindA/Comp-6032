...
class junctionDef:
    def __init__(
        self, x, y, cap, canStop, fareProb=None, maxTraffic=0, src=0, sink=0, **fareArgs
    ):
        self.x = x
        self.y = y
        self.capacity = cap
...
class NetWorld:
    ...
    def addNodes(self, nodes):
        ...
        if isinstance(nodes, list) or isinstance(nodes, tuple):
            try:
                invalid = next(
                    node for node in nodes if not isinstance(node, junctionDef)
                )
                print(
                    "Invalid node list to add to NetWorld graph: at least one object is not a junctionDef"
                )
                return
    ...
    def addEdges(self, edges, interpolate=False):
        ...
        if isinstance(edges, list) or isinstance(edges, tuple):
            try:
                invalid = next(
                    edge for edge in edges if not isinstance(edge, streetDef)
                )
                print(
                    "Invalid edge list to add to NetWorld graph: at least one object is not a streetDef"
                )
                return
            except StopIteration:
                ...
                if interpolate:
                    for edge in edges:
                        ...
                        if (
                            nextNodeIdx[0] == dst.index[0]
                            and nextNodeIdx[1] == dst.index[1]
                        ):
                            if (
                                self.addEdgeSegment(
                                    src, nextNodeIdx, edge.bidirectional
                                )
                                != dst
                            ):
                                raise KeyError(
                                    "Indexed node ({0},{1}) should be the end junction {2}".format(
                        ...
                        if not edge.bidirectional:
                            ...
                            if (
                                self.addEdgeSegment(
                                    penultimateNode, (dst.index[0], dst.index[1]), False
                                )
                                != dst
                            ):
                                raise KeyError(
                                    "Penultimate node {0} should be adjacent to end junction {1}".format(
            ...
            print(
                "Invalid edges argument to add to Networld graph: not a list or tuple"
            )
    ...
    def addEdgeSegment(self, start, end, bidir=True):
        ...
        if (
            nextIdx[0] > self.xSize - 1
            or nextIdx[0] < 0
            or nextIdx[1] > self.ySize - 1
            or nextIdx[1] < 0
        ):
            raise IndexError(
                "Next node out of range: ({0},{1})".format(nextIdx[0], nextIdx[1])
            )
                        ...
                        [
                            (
                                ...
                                (
                                    neighbour[0],
                                    self.distance2Node(
                                        node, self._net[(neighbour[1], neighbour[2])]
                                    ),
                                ),
                            )
    ...
    def travelTime(self, origin, destination):
        ...
        if (
            origin.traffic == origin.maxTraffic
            or destination.traffic == destination.maxTraffic
        ):
            return -1
        # all other destinations take finite time to reach.
            ...
            return round(
                (
                    origin.traffic
                    + destination.traffic
                    + self.distance2Node(origin, destination)
                )
                / 2
            )
    ...
    def distance2Node(self, origin, destination):
        ...
        if origin is None or destination is None:
            return -1
        return math.sqrt(
            (destination.index[0] - origin.index[0]) ** 2
            + (destination.index[1] - origin.index[1]) ** 2
        )
    ...
    def admitTaxi(self, node, admissionList):
        for taxi in admissionList.items():
            if taxi[1] not in self._taxis:
                raise ValueError(
                    "Node {0} tried to admit a taxi that does not exist".format(
                        node.index
                    )
                )
    ...
    def insertFare(self, node, fare):
        if node.index in self._fareQ:
            raise IndexError(
                "Node {0} generated a new fare for one where a Fare is already waiting".format(
                    node.index
                )
            )
        ...
        # Track fare generation in metrics
        if self._metrics is not None:
            self._metrics.record_fare_generated()
        return
    ...
    def removeFare(self, fare):
        ...
        if not fare.enroute:
            if self._dispatcher is not None:
                self._dispatcher.cancelFare(
                    self, fare.origin, fare.destination, fare.calltime
                )
            # Track fare cancellation in metrics
            if self._metrics is not None:
                self._metrics.record_fare_cancelled()
    ...
    def allocateFare(self, origin, taxi):
        ...
        taxi.recvMsg(
            taxi.FARE_ALLOC,
            **{"origin": origin, "destination": self._fareQ[origin].destination},
        )
    ...
    def completeFare(self, fare):
        taxi_revenue = fare.price * 0.9
        dispatcher_revenue = fare.price * 0.1

        self._dispatcher.recvPayment(self, dispatcher_revenue)
        fare.taxi.recvMsg(fare.taxi.FARE_PAY, **{"amount": taxi_revenue})
        print(
            f"Fare {fare.origin}->{fare.destination} completed! Taxi earned £{taxi_revenue:.2f}, Dispatcher earned £{dispatcher_revenue:.2f}"
        )
        # Track fare completion in metrics
        if self._metrics is not None:
            taxi_id = (
                fare.taxi.number if hasattr(fare.taxi, "number") else id(fare.taxi)
            )
            self._metrics.record_fare_completed(taxi_id=taxi_id, revenue=taxi_revenue)
    ...
    def runWorld(self, ticks=0, outputs=None, outlock=None):
        ...
        ticksRun = 0
        while (ticks == 0 or ticksRun < ticks) and (
            self.runTime == 0 or self._time < self.runTime
        ):
                    ...
                    if "taxis" in outputs:
                        ...
                        if taxi[1][0][0] is not None:
                            if taxi[0].number in outputs["taxis"]:
                                ...
                                outputs["taxis"][taxi[0].number][self._time] = taxi[1][
                                    0
                                ][0].index
                            else:
                                # outputs['taxis'][taxi[0].number] = {self._time: taxi[0].currentLocation}
                                outputs["taxis"][taxi[0].number] = {
                                    self._time: taxi[1][0][0].index
                                }
                    outlock.release()
            ...
            for node in self._trafficQ.items():
                self._trafficQ[node[0]] -= self._net[node[0]].injectTraffic(
                    self, node[1]
                )
