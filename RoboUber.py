...
def runRoboUber(
    ...
    **args,
    ...
    # Create metrics collector for this simulation run
    # Default algorithm is now 'astar' instead of 'baseline' to reflect A* implementation
    ...
    # create some taxis with optimized starting capital
    # Optimal configuration: 4 taxis with 1024 capital
    # This configuration avoids gridlock while maximizing completions
    ...
    while threadTime < threadRunTime:
        ...
        if stop.is_set():
            ...
            time.sleep(
                0.01
            )  # Reduced from 1s to 0.01s for faster simulation (100x speed increase!)

    # Simulation complete - finalize and save metrics
...
for run in range(numDays):
    ...
    while curTime < runTime - 1:
        ...
        # process pygame events once per loop to avoid consuming the event queue in multiple places
        events = pygame.event.get()
        # handle window close (clicking the X)
        for evt in events:
            if evt.type == pygame.QUIT:
                print("Window close requested. Exiting...")
                # signal the simulation thread to stop and allow it to proceed if it was waiting
                userExit.set()
                userConfirmExit.set()
                roboUber.join()
        ...
        # handle keyboard-driven quit (press 'q' then confirm with 'y')
        try:
            ...
            if quitevent.key == pygame.K_q:
                ...
                print(
                    "Really quit? Press Y to quit, any other key to ignore and keep running"
                )
            ...
            if (
                ...
                for jct in range(len(junctionIdxs)):
                    ...
                    pygame.draw.rect(
                        ...
                        pygame.Color(128, 128, 128),
                        pygame.Rect(
                            0, 0, round(meshSize[0] / 2), round(meshSize[1] / 2)
                        ),
                        5,
                    )
                    ...
                    for rfare in faresToRedraw.items():
                        ...
                        pygame.draw.polygon(
                            ...
                            [
                                ...
                                (
                                    meshSize[0] / 2
                                    - math.cos(math.pi / 6) * meshSize[1] / 4,
                                    meshSize[1] / 2
                                    + math.sin(math.pi / 6) * meshSize[1] / 4,
                                ),
                                (
                                    meshSize[0] / 2
                                    + math.cos(math.pi / 6) * meshSize[1] / 4,
...
if (
    fareFile is not None
):  # BUGFIX handle no open fare file to record ADR 21 October 2025
    fareFile.close()
pygame.quit()
sys.exit()
