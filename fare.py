...
class Fare:
    def __init__(
        self, parent, origin, destination, call_time, wait_time, cost_ceiling=None
    ):
        self._parent = parent
        self._origin = origin
    ...
    def setPrice(self, price):
        ...
        if (
            (expectedTime2Dest < 0)
            or (self._maxPrice is None and price > 10 * expectedTime2Dest)
            or (self._maxPrice is not None and price > self._maxPrice)
        ):
            print(
                "Fare ({0},{1}) abandoned because expectedTime2Dest was {2} and price was {3}".format(
                    self.origin[0], self.origin[1], expectedTime2Dest, price
                )
            )
            self._waitTime = 0
